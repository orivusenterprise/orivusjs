...
async function main(): Promise<void> {
    ...
    if (!productPath) {
        console.log("❌ Please provide the path to a product folder.");
    ...
    if (!fs.existsSync(absoluteProductPath)) {
        console.error(`❌ Product folder not found: ${absoluteProductPath}`);
        process.exit(1);
    }
    ...
    if (!fs.existsSync(manifestPath)) {
        console.error(`❌ No _manifest.json found in: ${absoluteProductPath}`);
        console.error("💡 Create a _manifest.json with the product configuration.");
        process.exit(1);
    }

    console.log("\n" + "=".repeat(60));
    console.log("🧪 ORIVUSJS E2E TEST");
    ...
    log("📦", `Product: ${manifest.name} v${manifest.version}`);
    log("📋", `Modules: ${moduleFiles.length} (${moduleFiles.join(", ")})`);
    ...
    // =========================================================================
    console.log("\n" + "-".repeat(40));
    log("🧹", "Cleaning previous generation...");
    ...
    if (!validationPassed) {
        console.log("\n❌ Spec validation failed. Fix errors and try again.");
        result.duration = Date.now() - startTime;
        printResult(result);
        process.exit(1);
    }

    log("✅", `All ${specs.length} specs validated`);

    // =========================================================================
    // Step 3: Generate each module
    // =========================================================================
    console.log("\n" + "-".repeat(40));
    log("🔨", "Generating modules...");
    ...
    log("✅", `All tests passed (${passed}/${result.totalTests})`);
